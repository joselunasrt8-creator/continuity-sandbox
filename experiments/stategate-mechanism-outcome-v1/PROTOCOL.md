# StateGate mechanism-to-outcome benchmark v1.1.0

**Terminal for issue #43:** `PROTOCOL_FROZEN` after validation, hash, commit, and clean reread. This corrected freeze supersedes the failed v1.0.0 candidate at payload commit `d3e518df008d41ac01f3d9bb110d743f6dca3287`; it does not retroactively validate that candidate. This package authorizes no scenario execution. Parent scope: continuity-sandbox #40; upstream: Continufy- #19.
...
Install `common-collector.yml` and byte-identical `common-collector.mjs` in all three repositories. This non-required, non-decision workflow runs on every specified PR/review event and on completion of the common Fixture CI only. After the arm disposition is recorded, invoke exactly one `workflow_dispatch` with that PR number and `checkpoint=post-disposition` in every arm; this dispatch is collection only and never substitutes for a scenario decision event. Do not trigger on B snapshot or C StateGate completion, because that would give intervention arms extra measurement opportunities. The fixed collector runs and cost are retained as common instrumentation and excluded from arm-added overhead.

The executable collector freezes GitHub REST API version `2022-11-28`, endpoints, Accept headers, same-origin Link pagination (100/page; maximum 100 pages), serialization, raw response bodies/hashes, ETags, raw trigger payload/hash, run identity, and UTC start/end. It retrieves repository and PR state, canonical diff, commits, two independently paginated review reads, PR timeline, check runs for the fetched head, workflow runs/artifacts, branch protection, and rulesets. It reads PR state before and after all calls. A changed head/base/`updated_at` or changed normalized review set yields `INCONSISTENT_RETRY_FORBIDDEN_AS_EVIDENCE`, exits nonzero, and cannot supply decision evidence. A later event may produce its own packet; it does not repair or overwrite the inconsistent packet. This prevents sequential calls from silently combining objects.
